package com.service.plugs.jpush;
...
public class JpushService {
    ...
    public PushResult sendCustomPush(String title, String content, Map<String, String> extrasMap, String... alias) {
        ...
        try {
            ...
            LOG.info("极光推送结果 - " + result+",接收推送的别名列表:" + JSONObject.toJSONString(alias) );
            ...
            LOG.info("以下存在不能识别的别名: " + JSONObject.toJSONString(alias) );
            LOG.error("Sendno: " + payload.getSendno());
        }
        return result;
    }
     ...
     *
     */
    public PushResult sendPush(String title, String content, String extrasMap, String... alias) {
        ClientConfig clientConfig = ClientConfig.getInstance();
        clientConfig.setTimeToLive(Long.valueOf(jpushConfig.getLiveTime()));
        // 使用NativeHttpClient网络客户端，连接网络的方式，不提供回调函数
        JPushClient jpushClient = new JPushClient(jpushConfig.getMasterSecret(), jpushConfig.getAppkey(), null,
        ...
        if(extrasMap!=null && !extrasMap.equalsIgnoreCase(""))
            extMap = JSON.parseObject(extrasMap,HashMap.class);
        PushPayload payload = buildPushPayload(title, content, extMap, alias);
        PushResult result = null;
        try {
            result = jpushClient.sendPush(payload);
    ...
    /**
     * 异步请求推送方式使用NettyHttpClient,异步接口发送请求，通过回调函数可以获取推送成功与否情况
     *
     * @param title
     *            通知栏标题
     * @param content
    ...
    private PushPayload buildPushPayload(String title, String content, Map<String, String> extrasMap, String... alias) {
        ...
        return PushPayload.newBuilder().setPlatform(Platform.android_ios())
                ...
                .setAudience((null == newAlias || newAlias.length == 0) ? Audience.all() : Audience.tag(alias))
